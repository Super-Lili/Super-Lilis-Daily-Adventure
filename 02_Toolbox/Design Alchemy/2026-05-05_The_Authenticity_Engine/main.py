...
def process(text: str) -> str:
    """
    Run the Authenticity Engine with default Lili settings and a demo content stream.
    Any non-empty text is treated as a comma-separated list of interests to add.
    """
    ...
    curated = lili.generate_curated_feed()

    lines = [f"--- {lili.username}'s Authenticity Engine Feed ---"]
    lines.append(f"Interests: {', '.join(lili.explicit_interests)}")
    lines.append(f"Trusted connections: {', '.join(lili.trusted_connections)}")
    lines.append("")
    if not curated:
        lines.append("Your curated feed is empty. Try adding more connections or interests!")
    else:
        for item in curated:
            tags_str = ', '.join(item.get('tags', []))
            lines.append(f"[{item.get('author_id', 'Unknown')}] - '{item.get('text', 'No content')}' (Tags: {tags_str})")
    lines.append("-" * 50)
    return "\n".join(lines)
...
def _cli_main():
    ...
    print(f"\n--- {lili.username}'s Authenticity Engine Feed (May 5, 2026) ---")
    curated = lili.generate_curated_feed()
    if not curated:
