...
- **F-034 (still unfixed as of this refresh)**: `_append_quality_ledger()` (`src/lili_validators.py:679`) still writes `datetime.utcnow()` directly instead of reading `LILI_DATE` — checked again this week (2026-09-02), no change. See FINDINGS F-034.

### Phase 15 — Evolution Truncation Fixed But Knobs Still Stuck, Duplicate-Run Bug Recurs With New Root Cause, CI Blind Spot Surfaces (2026-08-27 to 2026-09-02)

A week where several previously-"fixed" or "observed" mechanisms turned out to be incompletely closed, plus one genuinely new class of gap (a CI trigger blind spot) surfaced for the first time. No new tools shipped Wed-Fri due to a real, currently-ongoing infrastructure outage — see below, this needs owner action now.

- `b625e12` **Weekly evolution truncation fixed + honest rename** (F-035, renumbered from a duplicate F-034 this refresh corrected — the docs-refresh finding logged last week under F-034 was about the ledger date bug above; this is a separate truncation bug that got the same number by mistake and is now F-035): the last two weekly evolution runs (08-16, 08-23) silently truncated mid-response because `max_tokens=8192` wasn't enough for the ~14-section output format, and the old retry logic accepted any non-empty response as success. Fixed: `max_tokens` raised to 16384, `_is_truncated_evolution_response()` checks for the final `---END---` tag and triggers retry, falls back to the last partial response (not `None`) if all 3 retries still truncate. `call_gemini` also renamed to `call_deepseek_evolution` (a Gemini-era leftover name the owner caught by asking why Gemini was mentioned at all — the implementation had been pure DeepSeek since 2026-07-03). 203 tests total.
- **Open question, not yet resolved**: despite F-035 landing and two more weekly evolution runs completing since (08-30, 08-31), `lili_evolution_knobs.json`'s `week_start` is *still* stuck at `2026-08-03` — unchanged since before the truncation bug was even discovered. Cannot tell from repo state alone whether the truncation fix didn't fully work or the backtest gate (F-020) is correctly rejecting this week's proposals — the diagnostic that would answer this (`knobs_summary` in `super_lili_weekly_evolution.py`) is only ever printed to GitHub Actions stdout, never persisted to any file. See FINDINGS F-037.
- **Weekly evolution's back-to-back-duplicate-run bug recurred with a different root cause** (F-036): 08-30 and 08-31 both got a full weekly evolution report covering nearly the same week (24→30 and 25→31). This is NOT the 2026-08-10 UTC/Beijing mismatch (`58a7e35`) recurring — `LILI_DATE` is wired correctly and each run is internally self-consistent. The new cause: the 20:05 Beijing backup cron slot was delayed by GitHub Actions queueing over 4.5 hours (fired at UTC 16:40 instead of ~12:05), which pushed the `Check` step's `TZ=Asia/Shanghai date` computation across the Beijing midnight boundary into the next calendar day, so the skip guard's file-existence check looked for a report filename that (correctly, per that run's own clock) didn't exist yet. See FINDINGS F-036.
- **New CI blind spot found**: the test suite is currently red on `main` — `tests/test_prompts.py::test_daily_offender_concepts_injected_when_present` hardcodes a `"2026-08-01"` ledger fixture date and asserts it survives a 28-day real-wall-clock window (`lili_ledger_report.load_entries()` uses `datetime.now(timezone.utc)`, not an injectable time) — the fixture necessarily expired around 2026-08-29. This went unnoticed because commits authored by the weekly-evolution bot itself (`cb60884`, `2554fe9`, pushed via the workflow's default `GITHUB_TOKEN`) never trigger `lili_tests.yml`, even though they touch `src/**` — GitHub Actions does not fire `on: push` workflows for `GITHUB_TOKEN`-authenticated pushes, to prevent workflow loops. Confirmed via `gh run list`: no CI run exists for either commit, while a same-week human/Claude-pushed commit touching the same files (`b625e12`) triggered CI normally. This blind spot has existed since Phase 10 (self-evolution started writing `lili_soul.py`/`lili_engineering.py`/`lili_blindspot.py` weekly, 2026-07-17) — this is just the first time a test expired inside that unwatched window. Not fixed this session (docs-refresh scope only). See FINDINGS F-039.
- **Ongoing infrastructure outage, needs owner action now**: 2026-08-31, 09-01, and 09-02 diaries all show SCOUT rejected before it could run. DeepSeek genuinely returns `402 Insufficient Balance` (real billing outage, needs a top-up). Qwen returns `401 Incorrect API key provided` / `invalid_api_key` — this is a **credential problem, not a billing problem**, and topping up DeepSeek's balance will not fix it. `is_billing_error()`/`classify_scout_failure()` (F-024/F-025) correctly flag the day as an infrastructure outage overall (since DeepSeek's error does match billing signatures) and print the raw Qwen error text verbatim in the diary, but the generic "top up billing" guidance line is actively wrong for the Qwen side — owner needs to separately check/rotate the `QWEN_API_KEY` GitHub secret. See FINDINGS F-038.
- 71 tools in the toolbox as of 2026-09-02 (was 67 on 08-23): Decision Logic Extractor, Easing Curve Rosetta, Invoice Follow-Up Sequencer, Critique Memory Keeper.

### Phase 16 — Infrastructure Outage Escalates From Billing To Credentials, Weekly Evolution Silently Skipped For The First Time (2026-09-03 to 2026-09-06)

A full week with zero code changes and zero new tools — the Phase 15 outage did not resolve, it got worse and changed shape. Nothing in this section is a model-capability finding; all of it is the same live operational incident continuing.

- **DeepSeek's error flipped from billing to credentials, and the outage-visibility mechanism has no coverage for that**: between the 2026-09-02 20:36 UTC daily run (DeepSeek still `402 Insufficient Balance`, matching F-038's account) and the 2026-09-03 16:34 UTC run, DeepSeek's error changed to `401 - Authentication Fails, Your api key: ****1C08 is invalid`. Combined with Qwen's unchanged `401 invalid_api_key` (F-038), **both** providers now fail with credential errors that `is_billing_error()`'s keyword list doesn't recognize, so `classify_scout_failure()` falls through to the generic "Phase 1 failed" string and `save_rest_day()` renders 2026-09-04 and 2026-09-05 as an ordinary poetic "今天莉莉在休息" rest day — not the distinct INFRASTRUCTURE OUTAGE banner F-024/F-025 built specifically to make this unmissable. Confirmed via raw GitHub Actions logs (`gh run view --log`) for 4 independent runs across 2 different workflows: 2 daily runs (2026-09-04, 2026-09-05) and 2 weekly-evolution runs (2026-09-06, see next bullet). This is F-038's own inference ("a non-billing failure sitting alongside a billing failure gets its diagnosis masked") arriving in a worse form than predicted — not one provider masking another, but both failing outside the classifier's only recognized category at once. See FINDINGS F-040.
- **Weekly evolution failed outright for the first time — and has no failure-visibility mechanism at all**: the 2026-09-06 weekly evolution ran twice (00:48 UTC and 08:37 UTC), both hit the same DeepSeek `401 invalid key` error 3x, printed `❌ All models failed. Evolution postponed.`, and returned — `weekly_evolution()` has no `save_rest_day()` equivalent, so nothing was written anywhere: no `03_Evolution_Log/` file, no commit (the `git diff --cached --quiet ||` guard never fired), nothing but two lines in GitHub Actions stdout that no one but this refresh has read. Unlike the daily pipeline (which at least renders *some* diary, even if misclassified), a human skimming repo state has zero way to tell "evolution ran and failed" from "evolution never triggered." See FINDINGS F-040.
- **Consequence for last week's open question**: F-037 asked whether `lili_evolution_knobs.json`'s `week_start` staying stuck at `2026-08-03` was a truncation-fix failure or a correct backtest rejection. This week doesn't answer that — no evolution ran successfully at all, so there's simply no new data point either way. The knob file is unchanged.
...
- **Two weekly-evolution runs fired on back-to-back days** (2026-08-08 and 2026-08-09, commits `2168aa8` and `6fb0e58`) covering overlapping windows (07/27→08/08 in one report body vs. 08/03→08/09 in the other). **Root-caused and fixed 2026-08-10** (`58a7e35`): same UTC/Beijing mismatch as the diary-overwrite bug below — `lili_weekly_evolution.yml`'s skip guard checks for `03_Evolution_Log/{Beijing TODAY}_Weekly_Evolution.md`, but `weekly_evolution()` wrote the file using `datetime.utcnow()` with no override. The primary 07:05 Beijing slot (23:05 UTC Saturday) wrote a report dated one day behind Beijing, so the guard never found it and a backup slot later the same Beijing Sunday ran a redundant second full evolution pass — burning a real extra round of API calls. Not the ghost `workflow_dispatch` trigger from Phase 12; that remains separately unconfirmed. Fix: `weekly_evolution()` now reads `LILI_DATE` the same way `lili_pipeline.py`'s `evolve()` already did; the workflow passes the Check step's Beijing date through as `LILI_DATE` and reuses it for the commit message instead of recomputing.
- **The above bug's sibling silently overwrote a real diary entry**: `lili_daily.yml` had the identical mismatch — `lili_pipeline.py`'s `evolve()` already supported a `LILI_DATE` override, but the workflow never set it, so it fell back to UTC while the commit message and `TOOL_EXISTS`/throttle checks used Beijing time. On 2026-08-10 this finally caused real damage: the rest-day run (commit message "08-10") wrote to `01_Work_Log/2026-08-09-Diary.md` (UTC date), silently destroying the 2026-08-09 Weekly Evolution "Week in Review" content committed there hours earlier. History showed this exact mismatch had fired 11 times before, back to 2026-06-17, always previously landing on an empty file so the damage went unnoticed. Fixed 2026-08-10 (`6a781e4`) the same way as above; lost diary content restored from `6fb0e58`.
- **The back-to-back weekly-evolution duplicate-run bug came back with a different root cause**: the 2026-08-10 fix (`58a7e35`, above) made `LILI_DATE` internally consistent within a single run, but on 2026-08-30/08-31 the *backup* cron slot (20:05 Beijing) was itself delayed by GitHub Actions queueing over 4.5 hours, pushing its own `Check` step's date computation across the Beijing midnight boundary into the next calendar day — so that run's `TODAY` genuinely was "the next day" by its own (correct) clock, and the skip guard correctly found no report under that filename. Not yet fixed (docs-refresh scope). See FINDINGS F-036 for the full timestamp evidence. Any date-string-based idempotency guard on a schedule with widely-spaced retry slots has this risk — the fix likely needs to check "was any report written in the last N days," not "does today's exact filename exist."
- **A test suite regression can go undetected indefinitely if only the self-evolution bot's own commits trigger it**: `tests/test_prompts.py::test_daily_offender_concepts_injected_when_present` hardcoded a `"2026-08-01"` ledger fixture date checked against a real-wall-clock 28-day window (`lili_ledger_report.load_entries()` uses `datetime.now(timezone.utc)`, no injectable time) — it necessarily started failing once real time passed 2026-08-29. Confirmed still red locally as of 2026-09-02. It went unnoticed because GitHub Actions does not trigger `on: push` workflows for commits pushed with the default `GITHUB_TOKEN`, and every weekly-evolution commit (which touches `src/lili_soul.py`/`lili_engineering.py`/`lili_blindspot.py`, all under `lili_tests.yml`'s watched paths) is pushed that way — confirmed via `gh run list`, which shows no CI run for `cb60884`/`2554fe9` but a normal one for the same-week human/Claude-pushed `b625e12` touching the same files. This blind spot has existed since Phase 10 (2026-07-17), not introduced this week — this is just the first time a test happened to expire inside it. Not fixed this session. See FINDINGS F-039.
- **The billing-outage detector has no category for credential errors, and both providers fell into that gap at once**: `is_billing_error()`'s keyword list (`_BILLING_ERROR_SIGNATURES` in `src/lili_llm.py`) only recognizes billing/quota wording ("insufficient balance", "billing", etc.), not authentication wording ("invalid_api_key", "authentication_error", "incorrect api key provided"). F-038 (last week) already flagged Qwen's credential error as invisible to this classifier, but DeepSeek was still genuinely billing-broken (`402 Insufficient Balance`) at the time, so the `or` in `classify_scout_failure()` still produced the correct INFRASTRUCTURE OUTAGE banner overall. Sometime between 2026-09-02 20:36 UTC and 2026-09-03 16:34 UTC, DeepSeek's own error flipped to a credential error too (`401 Authentication Fails ... api key ... is invalid`) — with neither provider's error matching any billing signature, the classifier fell through to the generic "Phase 1 failed" string and 2026-09-04/09-05 rendered as ordinary creative rest days, not outage days. The same credential failure also silently killed the 2026-09-06 weekly evolution twice, and `weekly_evolution()` has no rest-day-equivalent write path at all — no file, no commit, nothing but two `print()` lines in GitHub Actions stdout. Not fixed this session (docs-refresh scope); needs the owner to rotate both `QWEN_API_KEY` and `DEEPSEEK_API_KEY` GitHub secrets, and ideally a follow-up to add a credential-error category to the classifier plus a failure-visibility write path for `weekly_evolution()`. See FINDINGS F-040.
